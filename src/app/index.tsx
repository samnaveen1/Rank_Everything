        ...
        <View style={[styles.workspace, isWideLayout && styles.wideWorkspace]}>
          ...
          <View style={styles.resultsPanel}>
            ...
            <View style={styles.listHeader}>
              <View>
                <Text style={styles.sectionKicker}>LIVE LEADERBOARD</Text>
                <Text style={styles.listTitle}>Your rankings</Text>
              </View>
              <Text style={styles.listCount}>
                {rankedItems.length > 0
                  ? `${Math.min(visibleCount, rankedItems.length)} of ${rankedItems.length} ${rankedItems.length === 1 ? "item" : "items"}`
                  : `0 items`}
              </Text>
            </View>
            <FlatList
              ...
              ListFooterComponent={hasMore ? (
                <Pressable onPress={() => setVisibleCount((c) => c + PAGE_SIZE)} style={styles.loadMoreButton}>
                  <Text style={styles.loadMoreText}>Load more ({rankedItems.length - visibleCount} remaining)</Text>
                </Pressable>
              ) : rankedItems.length > PAGE_SIZE ? (
                <View style={styles.pageInfo}>
                  <Text style={styles.pageInfoText}>All {rankedItems.length} items shown</Text>
                </View>
              ) : null}
            />
          </View>
...
const styles = StyleSheet.create({
  ...
  topBar: { flexDirection: "row", alignItems: "center", minHeight: 64, gap: 14, borderBottomWidth: 1, borderBottomColor: "#dce3e1", paddingBottom: 12 },
  headingBlock: { flexDirection: "row", alignItems: "center", gap: 10, minWidth: 215 },
  brandMark: { width: 32, height: 32, borderRadius: 8, backgroundColor: "#1e6b70", color: "#ffffff", textAlign: "center", textAlignVertical: "center", fontSize: 18, overflow: "hidden" },
  topSearch: { flex: 1, maxWidth: 430 },
  workspace: { flex: 1, gap: 14, paddingTop: 16 },
  wideWorkspace: { flexDirection: "row" },
  sidebar: { width: 140, padding: 12, borderRadius: 14, backgroundColor: "#ffffff", borderWidth: 1, borderColor: "#dfe5e3", alignSelf: "flex-start" },
  sidebarTitle: { color: "#65716f", fontSize: 10, fontWeight: "700", letterSpacing: 0.7, marginBottom: 10 },
  clearFilter: { marginTop: 14, alignItems: "center", paddingVertical: 6 },
  ...
  mobileAddButton: { position: "absolute", right: 18, bottom: 24, flexDirection: "row", alignItems: "center", gap: 6, paddingHorizontal: 18, paddingVertical: 14, borderRadius: 24, backgroundColor: "#1e6b70", elevation: 6 },
  ...
  eyebrow: { color: "#637084", fontSize: 10, fontWeight: "700", letterSpacing: 1, marginBottom: 3 },
  title: { fontSize: 18, fontWeight: "bold", color: "#17202b" },
  addButton: { backgroundColor: "#17232a", paddingHorizontal: 16, paddingVertical: 11, borderRadius: 11, minWidth: 76, alignItems: "center", elevation: 3 },
  addButtonText: { color: "#ffffff", fontWeight: "bold", fontSize: 12 },
  list: { paddingTop: 4, paddingBottom: 30 },
  gridRow: { gap: 10 },
  emptyList: { flexGrow: 1 },
  listHeader: { flexDirection: "row", alignItems: "flex-end", justifyContent: "space-between", marginTop: 4, marginBottom: 14 },
  sectionKicker: { color: "#65716f", fontSize: 10, fontWeight: "700", letterSpacing: 0.8, marginBottom: 4 },
  listTitle: { color: "#17202b", fontSize: 20, fontWeight: "700" },
  listCount: { color: "#7a8492", fontSize: 13 },
  loadMoreButton: { marginTop: 8, marginBottom: 20, paddingVertical: 14, borderRadius: 12, borderWidth: 1, borderColor: "#dfe1e7", backgroundColor: "#ffffff", alignItems: "center" },
  loadMoreText: { color: "#4d647c", fontSize: 13, fontWeight: "600" },
  pageInfo: { marginTop: 8, marginBottom: 20, alignItems: "center" },
  pageInfoText: { color: "#9aa3b0", fontSize: 12 },
  ...
  confirmDialog: { padding: 22, borderRadius: 18, backgroundColor: "#ffffff", elevation: 5 },
